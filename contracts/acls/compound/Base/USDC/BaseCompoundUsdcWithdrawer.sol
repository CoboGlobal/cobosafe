...
contract BaseCompoundUsdcWithdrawer is BaseCompoundAuthorizer {
    bytes32 public constant NAME = "BaseCompoundUsdcWithdrawer"; //
    ...
    function withdraw(address asset, uint amount) external view nonPayable onlycUSDbCv3 {
        _checkAsset(asset, USDC);
    }
    ...
    function invoke(bytes32[] calldata actions, bytes[] calldata data) external view nonPayable onlyBaseBulker {
        ...
        for (uint i = 0; i < actions.length; ) {
            ...
            if (action == ACTION_WITHDRAW_ASSET) {
                (address comet, address to, address asset, uint amount) = abi.decode(
                    data[i],
